...
const ALLOWED_FOCUS = new Set([
  'Operationskartläggning: AI i stabil drift',
  'AI-beredskapskartläggning',
  'First operation',
  'Managed operations',
  'Operating expansion',
  'Still exploring'
...
const ENTRY_TYPES = new Set(['paid', 'direct', 'internal_test']);
...
function validAcquisition(value, paid) {
  ...
  for (const [key, item] of Object.entries(value)) {
    ...
    if (key === 'entry_id') {
      if (!UUID_V4.test(item)) return false;
      continue;
    }
    if (key === 'entry_type') {
      if (!ENTRY_TYPES.has(item)) return false;
      continue;
    }
    if (key === 'entry_path') {
      if (!/^\/[a-z0-9/_-]{1,120}$/.test(item)) return false;
      continue;
    }
    if (key === 'consequences' || key === 'information_locations') {
      const items = typeof item === 'string' ? item.split(',').filter(Boolean) : [];
      if (!items.length || !items.every((entry) => ALLOWED_ACQUISITION[key].has(entry))) return false;
      continue;
    }
  ...
  if (paid) {
    const hasAllowedList = (key) => typeof value[key] === 'string' &&
      value[key].split(',').filter(Boolean).every((entry) => ALLOWED_ACQUISITION[key].has(entry));
    return value.source === 'meta_paid' &&
      hasAllowedList('consequences') &&
      hasAllowedList('information_locations') &&
